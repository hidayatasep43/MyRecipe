...
public class LocalPref {
    private static final String SETTINGS_NAME = "com.hidayatasep.myrecipe.pref";
    ...
    public void put(Key key, String[] values) {
        doEdit();
        StringBuilder str = new StringBuilder();
        for (String val : values) {
            str.append(val).append(",");
        }
        mEditor.putString(key.name(), str.toString());
        doCommit();
    }

    public void append(Key key, String values) {
        doEdit();
        StringBuilder str = new StringBuilder();
        String existing = getString(key);
        if(existing != null) str.append(existing);
